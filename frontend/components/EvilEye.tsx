...
void main() {
  ...
  float innerCore = clamp(innerRing + innerEye - pupil, 0.0, 3.0);
  float outerEdge = clamp(outerEyeGlow + outerBgGlow - (pupil * 0.55), 0.0, 3.0);
  float alpha = clamp(max(innerCore, outerEdge), 0.0, 1.0);
  vec3 color = (uEyeColor * innerCore + uOuterColor * outerEdge) * uIntensity;
  color = mix(uBgColor, color + uBgColor * 0.2, alpha);
...
export default function EvilEye({
  ...
  useEffect(() => {
    ...
    try {
      ...
      try {
        renderer = new Renderer({ alpha: true, premultipliedAlpha: false, webgl: 1 })
      } catch {
        // Some browsers/profiles disable WebGL; fail gracefully without crashing the page.
        return
      }
    }
  ...
  }, [backgroundColor, eyeColor, flameSpeed, glowIntensity, intensity, irisWidth, noiseScale, outerColor, pupilFollow, pupilSize, scale])
