...
export function GridScan({
  ...
  const pushScan = (t: number) => {
    ...
    if (materialRef.current) {
      const u = materialRef.current.uniforms as Record<string, { value: number[] | number }>
  ...
  useEffect(() => {
    ...
    const uniforms: Record<string, { value: THREE.Vector3 | number | THREE.Vector2 | THREE.Color | number[] }> = {
      iResolution: {
  ...
  useEffect(() => {
    ...
    if (m) {
      const u = m.uniforms as Record<string, { value: THREE.Color | number | number[] }>
    ...
    if (bloomRef.current) {
      ...
      const lum = (bloomRef.current as unknown as { luminanceMaterial?: { threshold: number; smoothing: number } })
        .luminanceMaterial
      if (lum) {
        lum.threshold = bloomThreshold
        lum.smoothing = bloomSmoothing
      }
    }
  ...
  return (
    ...
    >
      {showPreview && enableWebcam && (
        <div className="absolute right-3 bottom-3 w-[220px] h-[132px] rounded-lg overflow-hidden border border-white/25 shadow-[0_4px_16px_rgba(0,0,0,0.4)] bg-black text-white text-[12px] leading-[1.2] font-sans pointer-events-none flex items-center justify-center">
          <span className="px-2 py-1 bg-black/50 rounded backdrop-blur-[4px]">Webcam disabled (install face-api.js to enable)</span>
        </div>
      )}
    </div>
  )
