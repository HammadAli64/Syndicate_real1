...
export default function SignupPage() {
  ...
  async function onSubmit(event: FormEvent) {
    ...
    setError("");
    try {
      const response = await fetch(`${API_BASE_URL}/api/syndicate-auth/signup/`, {
        method: "POST",
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-black px-4 text-white">
      <div className="w-full max-w-md rounded-xl border border-[rgba(255,215,0,0.35)] bg-black/70 p-7 shadow-[0_0_35px_rgba(255,215,0,0.15)]">
        <h1 className="text-lg font-black uppercase tracking-[0.18em] text-[color:var(--gold)]">Sign up</h1>
        <p className="mt-1 text-xs text-white/55">Simple email/password signup.</p>
        ...
        <div className="mt-4 text-center">
          <Link href="/login" className="text-xs text-white/70 underline-offset-4 hover:text-[color:var(--gold)] hover:underline">
